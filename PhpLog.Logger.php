...
class CLogger {
    ...
    public function log($message, $level = 'info', $category = 'application') {
        ...
        $this->_logCount++;
        if ($this->autoDump || ($this->autoFlush > 0 && $this->_logCount >= $this->autoFlush && !$this->_processing)) {
    ...
    public function getLogs($levels = '', $categories = array() , $except = array()) {
        ...
        $ret = $this->_logs;
        if (!empty($levels)) $ret = array_values(array_filter($ret, array(
            $this,
            'filterByLevel'
        )));
        if (!empty($this->_categories) || !empty($this->_except)) $ret = array_values(array_filter($ret, array(
            $this,
            'filterByCategory'
        )));
        return $ret;
